...
enum Elem { E_NONE = -1, E_SL_L, E_SL_R, E_TGL_L, E_TGL_R, E_BTN_KEY, E_BTN_KEEP, E_MODE_L, E_MODE_R, E_COUNT };
...
// ---- layout rects ----
struct LY {
    RECT title, card[4], track[2], thumb[2];
...
static int  ThumbX(int i) {
    ...
    float r = (float)(c - CPS_MIN10) / (CPS_MAX10 - CPS_MIN10);
    return L.track[i].left + (int)(r * (L.track[i].right - L.track[i].left));
}
static void UpThumbs() { for (int i = 0; i < 2; i++) { int cx = ThumbX(i); L.thumb[i].left = cx - 14; L.thumb[i].right = cx + 14; g_hr[i].r = L.thumb[i]; } }
...
static void Paint()
{
    ...
    SelectObject(dc, g_hfBody);
    {
        ...
        wchar_t buf[128];
        swprintf(buf, 128, L"连点器: 左 %.1f cps (%d ms)  右 %.1f cps (%d ms)",
                 lc, leftms, rc, rightms);
        SetTextColor(dc, CLR_TEXT_DIM);
...
static void Drag(int i, int mx)
{
    ...
    if (multimode) {
        ...
        int c10 = CPS_MIN10 + (int)(r * (CPS_MAX10 - CPS_MIN10));
        if (c10 < CPS_MIN10) c10 = CPS_MIN10; if (c10 > CPS_MAX10) c10 = CPS_MAX10;
...
static void Click(HWND hwnd, Elem e)
{
    ...
    doModeSwitch:
        multimode = !multimode;
        ShowToast(multimode ? L"\x591a\x500d\x70b9\x6a21\x5f0f" : L"\x8fde\x70b9\x5668\x6a21\x5f0f",
                  L"\x5df2\x5207\x6362", CLR_ACCENT);
        break;
    default: break;
    }
